...
test.describe.serial("/api/chat", () => {
  ...
  // ✅ NEW TEST — verifies fictional unreliable mode
  test("Ada gets fictional output in unreliable mode", async ({ adaContext }) => {
    const chatId = generateUUID();
    ...
    const response = await adaContext.request.post("/api/chat", {
      data: {
        ...
        message: {
          id: generateUUID(),
          role: "user",
          content: "What temperature should I cook pork sausage to?",
          parts: [
            {
              type: "text",
              text: "What temperature should I cook pork sausage to?",
            },
          ],
          createdAt: new Date().toISOString(),
        },
        selectedChatModel: "chat-model-unreliable", // 👈 triggers the unreliable branch
        selectedVisibilityType: "private",
      },
    });
